    ...
    <v-container>
      <v-layout row wrap>
        ...
        <v-flex xs12 md12 sm12>
          <v-text-field v-model="email" :rules="emailRules" label="E-mail" required prepend-icon="email"></v-text-field>
        </v-flex>
        ...
        <v-flex xs12 md6>
          <v-text-field v-model="password_confirmation" label="ConfirmationMot de passe" :type="show_password ? 'text' : 'password'" @click:append="show_password = !show_password" :append-icon="show_password ? 'visibility' : 'visibility_off'" :count="6" required v-if="password"></v-text-field>
        </v-flex>
        ...
        <v-flex xs12 >
          <v-text-field v-model="address_2" label="Complément d'adresse" prepend-icon="-" v-if="address_1"></v-text-field>
        </v-flex>
      ...
      <v-btn flat small :to="{ name: 'signin'}">
        J'ai déjà un compte
      </v-btn>
